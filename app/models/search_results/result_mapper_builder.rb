module SearchResults
  ...
  class ResultMapperBuilder
    ...
    def strategy_mapper_class
      "SearchResults::ResultMapper#{strategy_mapper.to_s.camelize}".constantize
    end
    ...
    def self.strategy_class_builder(result_type)
      "SearchResults::#{strategy_builder(result_type).to_s.camelize}GroupsBuilder".constantize
    end

    def self.build_result_groups(result_adapter)
      strategy_class_builder(result_adapter.type).new(result_adapter).build_result_groups
    end
  ...
  class CompareGroupsBuilder < GroupsBuilder
    def build_result_groups
      vals = LingsProperty.with_id(vals_ids_for_compare)
    ...
    def vals_ids_for_compare
      is_parent?(depth_for_compare) ? parent_ids : child_ids
    end
