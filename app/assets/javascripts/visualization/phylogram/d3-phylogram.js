...
if (!d3) { throw new Error("d3 wasn't included!"); }
(function() {
  d3.phylogram = {};
  ...
  d3.phylogram.build = function(selector, nodes, options) {
    ...
    function getNodeLength(d){
      return !options.skipLabels ? '' : d.length;
    }

    function getNodeFullName(d){
      return d.name + (!options.skipLengthLabels ? '' : ' ('+ d.length+')');
    }
  ...
  d3.phylogram.buildRadial = function(selector, nodes, options) {
    ...
    var phylogram = d3.phylogram.build(selector, nodes, {
      ...
      skipLengthLabels: options.skipLengthLabels,
