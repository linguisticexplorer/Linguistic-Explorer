class GeoMapping
  ...
  def initialize(ids)
    ...
    Rails.logger.debug "[DEBUG] #{ids.inspect}"
    # @data = { lings: {}, titles: {}, colours: {} }
    # create_lings_hash(search)
    
    # find latlong property id
    latlong_id = Property.in_group(current_group).where(:name => "latlong").first
    
    # perform a query and save id => latlong association
    entries = LingsProperty.
      in_group(current_group).
      select(:id, :property_value).
      where(:property_id => latlong_id, :ling_id => ids)

    Rails.logger.debug "[DEBUG] #{entries.inspect}"

    @lingCoords = entries.map do |e| 
      {:id => e.ling_id, :lat => e.property_value.split(',').first, :long => e.property_value.split(',').last }
    end
  end

  def build_json
    @lingCoords.to_json.html_safe
  end
